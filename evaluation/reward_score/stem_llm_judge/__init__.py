...
# ------------ Core LLM call --------------------------------------------------
async def _llm_judge(question: str, student: str, reference: str, verbose: bool = False, use_gpt: bool = False) -> Tuple[bool, str]:
    ...
    # base_url = "http://176.56.200.81:30000"
    if not use_gpt:
        base_url = os.getenv("STEM_LLM_JUDGE_URL")
        if not base_url:
            raise ValueError("STEM_LLM_JUDGE_URL is not set")
        evaluation_llm_client = AsyncOpenAI(base_url=base_url,
                                            api_key='EMPTY')
        model = os.getenv('STEM_LLM_JUDGE_MODEL', 'Qwen/Qwen3-32B')
    else:
    ...
    "Sometimes the student's answer is expressed in a different way from the ground truth answer, but the meaning is the same, and you should still consider it correct. If they are not equivalent in mathematical sense, you should consider it incorrect.\n"
    ...
    # resp = await evaluation_llm_client.chat.completions.create(
    #     model="Qwen/Qwen3-32B",
    #     messages=[{"role": "user", "content": prompt}],
    #     max_tokens=8192,
    #     temperature=0.1,
    ...
    # # print("STEM LLM Judge Response: ", text)
    ...
    for i in range(3):
        try:
            ...
            print(f"Error in STEM LLM Judge: {e}")
            resp = None
            score = 0.0
...
async def compute_score(data_source: str,
                  ...
                  ground_truth: str,
                  extra_info: dict,
                  use_gpt: bool = False) -> Tuple[bool, float, str]:
    """
    Arguments
    ---------
    ...
    if is_matched==False:
        ...
        try:
            is_correct, content = await _llm_judge(question, extracted_model_output, ground_truth, verbose=False, use_gpt=use_gpt)
...
if __name__ == "__main__":
    ...
    score = asyncio.run(compute_score(
        data_source="",
