...
# Put your final grade in the following format:
...
async def llm_check_answer(model_output: str, ground_truth: str, question: str, use_gpt: bool = False) -> Tuple[bool, str]:
    ...
    if not use_gpt:
        base_url = os.getenv("MATH_LLM_JUDGE_URL")
        if not base_url:
            raise ValueError("MATH_LLM_JUDGE_URL is not set")
        evaluation_llm_client = AsyncOpenAI(base_url=base_url,
                                            api_key='EMPTY')
        model = os.getenv('MATH_LLM_JUDGE_MODEL', 'Qwen/Qwen3-32B')
    ...
    # print("Math LLM Judge Response: ", content)
    # eval_result = not "INCORRECT" in content \
    #     and "CORRECT" in content
    # eval_result = not "incorrect" in content.lower() \
    #     and "correct" in content.lower()
    # print({
    ...
    for i in range(3):
        try:
            ...
            print(f"Error in Math LLM Judge: {e}")
            eval_result = False
            content = str(e)
...
async def compute_score(model_output: str,
                  ground_truth: str,
                  extra_info: dict,
    ...
    content = 'Graded by simple rules'
    if grade_answer(extracted_model_output, ground_truth):
        # return True, True, extracted_model_output, 'Graded by simple rules'
        return {
            "score": 1.0,
            "is_correct": True,
    ...
    if is_matched and not is_correct:
        # use llm to check if the answer is correct
        is_correct, content = await llm_check_answer(extracted_model_output, ground_truth, question, use_gpt=use_gpt)
